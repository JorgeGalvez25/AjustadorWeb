...
uses
  ...
  Dialogs, StdCtrls, cxControls, cxContainer, cxEdit, cxTextEdit, UFIBCLASS2,
  cxMaskEdit, cxDropDownEdit, cxCalendar, RpDefine, RpRave,
  RpBase, RpSystem, RpRender, RpRenderPDF, cxSpinEdit;
...
type
  TFRep02 = class(TForm)
    ...
    CheckBox12a12: TCheckBox;
    CheckBox6a6: TCheckBox;
    procedure FormCreate(Sender: TObject);
    procedure btnCancelarClick(Sender: TObject);
    procedure btnAceptarClick(Sender: TObject);
    procedure CheckBox12a12Click(Sender: TObject);
    procedure CheckBox6a6Click(Sender: TObject);
...
var
  FRep02: TFRep02;
  nombreEst:string;
  bloquea12a12:Boolean;

implementation

uses DateUtils, Math, UAJUMENU;
...
//  RvSystem1.OutputFileName := Ruta;
  ...
  for i:=1 to 3 do begin
     ...
     case i of
       1: begin
            RvProject1.SetParam('magna',_NOMBRESCOMBUSTIBLES[1]);
       ...
       2: begin
            RvProject1.SetParam('premium',_NOMBRESCOMBUSTIBLES[2]);
       ...
       3: begin
            RvProject1.SetParam('diesel',_NOMBRESCOMBUSTIBLES[3]);
...
function GetPrecioCumbustible(const ACombustible: Integer; const AFecha:TDateTime): Double;
var
  Q_Claves: TADIQuery;
begin
  try
    Q_Claves := TADIQuery.Create('SELECT PRECIO FROM CLAVES WHERE COMBUSTIBLE = :COMBUSTIBLE AND FECHA <= :FECHA ORDER BY FECHA DESC ROWS 1');
...
begin
  try
    Q_Claves := TADIQuery.Create('SELECT SUM(VOLUMEN1 - VOLUMEN2) AS AJUSTE FROM CLAVES WHERE FECHAADMIN=:FECHAADMIN AND COMBUSTIBLE=:COMBUSTIBLE');
    Q_Claves.ParamByName('FECHAADMIN').AsDateTime := AFecha;
    Q_Claves.ParamByName('COMBUSTIBLE').AsInteger := ACombustible;
    Q_Claves.ExecQuery;
...
function ObtenerAjusteAdmin24(AFecha: TDateTime; ACombustible: Integer): Double;
var
  Q_Claves: TADIQuery;
begin
  try
...
//  RvProject1.SetParam('Empresa', '');
  RvProject1.SetParam('Empresa',nombreEst);
  ...
  for i := 1 to 3 do begin
    ...
    pPrecio := GetPrecioCumbustible(i,edtFechaIni.Date);

    try
      if CheckBox12a12.Checked then begin
//        Q_Rep :=  TADIQuery.Create('SELECT * FROM DAME_AJUSTE_ADMIN_GLOBAL(:FECHA, :COMBUSTIBLE)');
...
//        Q_Rep.ExecQuery;
        ...
        pVolumen := ObtenerAjusteAdmin24(edtFechaIni.Date, i);
...
procedure TFRep02.CheckBox12a12Click(Sender: TObject);
begin
  CheckBox6a6.Checked:=not CheckBox12a12.Checked;
end;

procedure TFRep02.CheckBox6a6Click(Sender: TObject);
begin
  CheckBox12a12.Checked:=not CheckBox6a6.Checked;
end;

procedure TFRep02.FormShow(Sender: TObject);
begin
  if bloquea12a12 then begin
    CheckBox12a12.Visible:=False;
    CheckBox6a6.Checked:=True;
    CheckBox6a6.Enabled:=False;
  end;
end;
